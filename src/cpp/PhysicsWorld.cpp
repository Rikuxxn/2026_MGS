...
void PhysicsWorld::ProjectOBB(const D3DXVECTOR3& axis, BoxCollider* obb, float& outMin, float& outMax)
{
    ...
    const D3DXVECTOR3& h = obb->GetScaledSize() * CMathConstant::HALF;
...
D3DXVECTOR3 PhysicsWorld::ClosestPointOnOBB(const D3DXVECTOR3& point, BoxCollider* obb)
{
    ...
    D3DXMATRIX R = obb->GetRotation();
    D3DXVECTOR3 half = obb->GetScaledSize() * CMathConstant::HALF;
...
{
    D3DXVECTOR3 aTop = a->GetPosition() + D3DXVECTOR3(0, a->GetHeight() * CMathConstant::HALF, 0);
    D3DXVECTOR3 aBottom = a->GetPosition() - D3DXVECTOR3(0, a->GetHeight() * CMathConstant::HALF, 0);
    D3DXVECTOR3 bTop = b->GetPosition() + D3DXVECTOR3(0, b->GetHeight() * CMathConstant::HALF, 0);
    D3DXVECTOR3 bBottom = b->GetPosition() - D3DXVECTOR3(0, b->GetHeight() * CMathConstant::HALF, 0);
...
{
    // カプセルの上下点
    D3DXVECTOR3 capTop = cap->GetPosition() + D3DXVECTOR3(0, cap->GetHeight() * CMathConstant::HALF, 0);
    D3DXVECTOR3 capBottom = cap->GetPosition() - D3DXVECTOR3(0, cap->GetHeight() * CMathConstant::HALF, 0);
    ...
    // もう一回OBBに投影
    closestBox = ClosestPointOnOBB(closestCapsule, box);
...
bool PhysicsWorld::CylinderBoxCollision(CylinderCollider* cyl, BoxCollider* box, D3DXVECTOR3& outPush)
{
    ...
    float halfHeight = cyl->GetHeight() * CMathConstant::HALF;

    // Box 情報（OBB対応）
    D3DXVECTOR3 boxCenter = box->GetPosition();
    const D3DXMATRIX& boxRot = box->GetRotation();
    D3DXVECTOR3 boxHalfExtents = box->GetScaledSize() * CMathConstant::HALF;
...
bool PhysicsWorld::CylinderCapsuleCollision(CylinderCollider* cyl, CapsuleCollider* cap, D3DXVECTOR3& outPush)
{
    ...
    // シリンダー線分（上下中心）
    D3DXVECTOR3 cylTop = cylPos + D3DXVECTOR3(0, cylH * CMathConstant::HALF, 0);
    D3DXVECTOR3 cylBottom = cylPos - D3DXVECTOR3(0, cylH * CMathConstant::HALF, 0);

    // カプセル線分（上下中心）
    D3DXVECTOR3 capTop = capPos + D3DXVECTOR3(0, capH * CMathConstant::HALF, 0);
    D3DXVECTOR3 capBottom = capPos - D3DXVECTOR3(0, capH * CMathConstant::HALF, 0);
...
bool PhysicsWorld::SphereBoxCollision(SphereCollider* sphere, BoxCollider* box, D3DXVECTOR3& outPush)
{
    ...
    const D3DXMATRIX& boxRot = box->GetRotation();
    const D3DXVECTOR3 boxHalfExtents = box->GetScaledSize() * CMathConstant::HALF; // サイズの半分
...
{
    // カプセル軸の両端点を取得（ワールド座標）
    D3DXVECTOR3 p1 = capsule->GetPosition() + D3DXVECTOR3(0, capsule->GetHeight() * CMathConstant::HALF, 0); // 上側
    D3DXVECTOR3 p2 = capsule->GetPosition() - D3DXVECTOR3(0, capsule->GetHeight() * CMathConstant::HALF, 0); // 下側
...
D3DXVECTOR3 PhysicsWorld::GetActualCollisionPoint(RigidBody* a, RigidBody* b, const D3DXVECTOR3& push)
{
    ...
    D3DXVECTOR3 halfA = a->GetScale() * CMathConstant::HALF;
    D3DXVECTOR3 halfB = b->GetScale() * CMathConstant::HALF;
    ...
    // 中点を返す
    return (contactA + contactB) * CMathConstant::HALF;
}
//=============================================================================
// 当たり判定シミュレーション
//=============================================================================
...
// ペア生成時に順番固定する処理
//=============================================================================
PhysicsWorld::CollisionPair PhysicsWorld::MakePair(Collider* a, Collider* b)
{
    if (a < b)
    {
        return { a, b };
...
void PhysicsWorld::RemoveCollider(Collider* col)
{
    ...
    // prev
    for (auto it = m_prevCollisions.begin(); it != m_prevCollisions.end(); )
    {
        if (it->a == col || it->b == col)
        {
