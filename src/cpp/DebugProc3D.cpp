...
void CDebugProc3D::DrawBoxCollider(BoxCollider* box, D3DXCOLOR color)
{
    ...
    // 半分サイズ取得
    D3DXVECTOR3 half = box->GetScaledSize() * CMathConstant::HALF;
...
void CDebugProc3D::DrawCapsuleCollider(CapsuleCollider* capsule, D3DXCOLOR color)
{
    ...
    {
        float theta1 = (CMathConstant::F_DOUBLE * D3DX_PI * nCnt) / kNumSegments;
        float theta2 = (CMathConstant::F_DOUBLE * D3DX_PI * (nCnt + 1)) / kNumSegments;
    ...
    {
        float theta1 = (D3DX_PI * CMathConstant::HALF) * (ring / (float)kNumRings);       // 0 ～ π/2
        float theta2 = (D3DX_PI * CMathConstant::HALF) * ((ring + 1) / (float)kNumRings);

        for (int seg = 0; seg < kNumSegments; seg++)
        {
            float phi1 = (CMathConstant::F_DOUBLE * D3DX_PI * seg) / kNumSegments;
            float phi2 = (CMathConstant::F_DOUBLE * D3DX_PI * (seg + 1)) / kNumSegments;
    ...
    for (int ring = 0; ring < kNumRings; ring++)
    {
        float theta1 = (D3DX_PI * CMathConstant::HALF) * (ring / (float)kNumRings);
        float theta2 = (D3DX_PI * CMathConstant::HALF) * ((ring + 1) / (float)kNumRings);
...
void CDebugProc3D::DrawCylinderCollider(CylinderCollider* cylinder, D3DXCOLOR color)
{
    ...
    float halfH = cylinder->GetHeight() * CMathConstant::HALF;

    // 上下円の頂点を計算
    for (int nCnt = 0; nCnt < kSegments; nCnt++)
    {
        float theta1 = (CMathConstant::F_DOUBLE * D3DX_PI * nCnt) / kSegments;
        float theta2 = (CMathConstant::F_DOUBLE * D3DX_PI * (nCnt + 1)) / kSegments;
...
void CDebugProc3D::DrawSphereCollider(SphereCollider* sphere, D3DXCOLOR color)
{
    ...
    for (int nCnt = 0; nCnt < kSegments; nCnt++)
    {
        float t1 = (CMathConstant::F_DOUBLE * D3DX_PI * nCnt) / kSegments;
        float t2 = (CMathConstant::F_DOUBLE * D3DX_PI * (nCnt + 1)) / kSegments;
