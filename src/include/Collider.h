...
public:
    ...
    IGameObject* GetOwner(void) const { return m_owner; }
...
{
public:
    BoxCollider(const D3DXVECTOR3& size) : Collider(BOX), 
        m_Size(size) 
    {}
...
{
public:
    CapsuleCollider(float radius, float height) : Collider(CAPSULE), 
        m_Radius(radius), 
        m_Height(height) 
    {}
    ...
    float       GetHalfHeight(void) const { return m_Height * CMathConstant::HALF; }
...
{
public:
    CylinderCollider(const D3DXVECTOR3& size, const D3DXVECTOR3& dir) : Collider(CYLINDER), 
        m_Size(size),
        m_Dir(dir)
    {
        m_Radius = size.x * CMathConstant::HALF;
        m_Height = size.y;
    }
...
{
public:
    SphereCollider(const D3DXVECTOR3& size) : Collider(SPHERE),
        m_Size(size),
        m_Radius(std::max({ size.x, size.y, size.z })* CMathConstant::HALF),
